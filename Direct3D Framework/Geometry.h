...
// 光線と平面の交差点までの距離を計算する
inline float DistanceToRayPlaneIntersection(
	const DirectX::SimpleMath::Vector3& point,				// 位置
	const DirectX::SimpleMath::Vector3& rayHeading,	// 進行方向ベクトル
	const DirectX::SimpleMath::Vector3& planePoint,		// 平面上の位置
	const DirectX::SimpleMath::Vector3& planeNormal	// 平面の法線
)
{
	// 進行ベクトルが直線と平行の場合
	if (planeNormal.Dot(rayHeading) < 0.001f &&  planeNormal.Dot(rayHeading) > -0.001f)
	{
		return -1.0f;
	}

	// 光線の式: P = P0 + tV
	// 平面の式: ax + by + cz + d = 0 → P・N + d = 0 
	// t = -(P0・N + d) / (V・N)
	float d = -planePoint.Dot(planeNormal);
	// レイと直線との距離を計算する( -(P0・N + d) / (V・N))
	return -(point.Dot(planeNormal) + d) / rayHeading.Dot(planeNormal);
}
